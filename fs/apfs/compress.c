...
#define MAX_FBUF_SIZE		(4096 * 1024)
...
static int apfs_compress_file_open(struct inode *inode, struct file *filp)
{
	...
	if(apfs_compress_is_rsrc(fd->hdr.algo)) {
		...
		if(fd->hdr.size > MAX_FBUF_SIZE)
			goto fail_enomem;

		fd->size = fd->hdr.size;
		fd->data = kvmalloc(fd->hdr.size, GFP_KERNEL);
		if(!fd->data)
			goto fail_enomem;
...
static ssize_t apfs_compress_file_read_block(struct apfs_compress_file_data *fd, char __user *buf, size_t size, loff_t off)
{
	...
	if(off >= fd->hdr.size)
		return 0;
	if(size > fd->hdr.size - off)
		size = fd->hdr.size - off;
	...
	if(block != fd->bufblk) {
		...
		if(doffs >= fd->size || fd->size - doffs < sizeof(*cd))
			return -EINVAL;
		cd = fd->data + doffs;
		if(fd->size - doffs - sizeof(*cd) < sizeof(cd->block[0]) * (size_t)cd->num)
			return -EINVAL;

		if(block >= cd->num)
			return 0;

		bsize = fd->hdr.size - block * APFS_COMPRESS_BLOCK;
		if(bsize > APFS_COMPRESS_BLOCK)
			bsize = APFS_COMPRESS_BLOCK;

		csize = cd->block[block].size;
		coffs = cd->block[block].offs + 4;
