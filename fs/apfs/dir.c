				...
				int use_hash)
{
	...
	if(use_hash)
		namelen = query->key_len - sizeof(*de_hkey);
	else
		namelen = query->key_len - sizeof(*de_ukey);
					     ...
					     struct apfs_drec *drec)
{
	...
	apfs_init_drec_hashed_key(sb, cnid, child->name, &key, sbi->use_hash);
	...
	do {
		...
		if (err)
			goto fail;
		err = apfs_drec_from_query(query, drec, sbi->use_hash);
		if (err)
			goto fail;
...
static int apfs_readdir(struct file *file, struct dir_context *ctx)
{
	...
	apfs_init_drec_hashed_key(sb, cnid, NULL /* name */, &key, sbi->use_hash);
	query->key = &key;
...
/**
 * apfs_build_dentry_hashed_key - Allocate and initialize the key for a dentry record
 * @qname:	filename
 * @hash:	filename hash
...
/**
 * apfs_build_dentry_plain_key - Allocate and initialize the key for a dentry record
 * @qname:	filename
 * @parent_id:	inode number for the parent of the dentry
 * @key_p:	on return, a pointer to the new on-disk key structure
 *
 * Returns the length of the key, or a negative error code in case of failure.
 */
static int apfs_build_dentry_plain_key(struct qstr *qname, u64 parent_id,
				 struct apfs_drec_key **key_p)
{
	...
	apfs_key_set_hdr(APFS_TYPE_DIR_REC, parent_id, key);
	key->name_len = cpu_to_le32(namelen);
	strcpy(key->name, qname->name);
				  ...
				  u64 parent_id, u64 sibling_id)
{
	...
	struct apfs_key key;
	struct apfs_query *query;
	struct apfs_drec_hashed_key *raw_hkey = NULL;
	struct apfs_drec_key *raw_ukey = NULL;
	struct apfs_drec_val *raw_val = NULL;
	int key_len, val_len;
	int ret;

	apfs_init_drec_hashed_key(sb, parent_id, qname->name, &key, sbi->use_hash);
	query = apfs_alloc_query(sbi->s_cat_root, NULL /* parent */);
	if (!query)
		return -ENOMEM;
	query->key = &key;
	...
	if(sbi->use_hash)
		key_len = apfs_build_dentry_hashed_key(qname, key.number, parent_id, &raw_hkey);
	else
		key_len = apfs_build_dentry_plain_key(qname, parent_id, &raw_ukey);
	if (key_len < 0) {
		ret = key_len;
		goto fail;
	}
	...
	ret = apfs_btree_insert(query, sbi->use_hash ? (void *)raw_hkey : (void *)raw_ukey, key_len, raw_val, val_len);
