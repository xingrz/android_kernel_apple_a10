 ...
 */
static void apfs_inode_set_ops(struct inode *inode, dev_t rdev, int is_compressed)
{
...
static int apfs_inode_from_query(struct apfs_query *query, struct inode *inode)
{
	...
	if((inode_val->bsd_flags & APFS_INOBSD_COMPRESSED) && !S_ISDIR(inode->i_mode)) {
	...
	apfs_inode_set_ops(inode, rdev, is_compressed);
	return 0;
...
struct inode *apfs_new_inode(struct inode *dir, umode_t mode, dev_t rdev)
{
	...
	apfs_inode_set_ops(inode, rdev, 0);
	return inode;
}
